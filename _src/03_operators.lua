...
--   - 逻辑：&& || !（短路求值）
--   - 比较：== != < <= > >=
--   - 无三目运算符，if 是表达式（let x = if cond { a } else { b }）
...
-- C++:
--   - 算术：+ - * / %（整数除法 5/2=2，浮点除法 5.0/2.0=2.5）
--   - 位运算：& | ^ ~ << >>
--   - 逻辑：&& || !（短路求值）
--   - 比较：== != < <= > >=
--   - 三目运算符：cond ? a : b
...
function compare_operators()
    ...
    -- Go: 5 / 2 = 2 (整数除), float64(5) / 2 = 2.5
    -- C++: 5 / 2 = 2 (整数除), 5.0 / 2.0 = 2.5
    ...
    -- 0 是真（Lua 独有）
    print("0 and true (Lua):", 0 and true)  -- true（Lua 独有）
end

-- ============================================================
-- 【练习题】
-- ============================================================
-- 1. 实现一个函数，用位运算判断一个数是否是 2 的幂（返回 true/false）
-- 2. 比较 and/or 的返回机制与 Python 的 and/or 的区别，并说明 0 在 Lua 中的布尔判断
-- 3. 解释为什么 Lua 的 ~= 表示不等于（而不是 !=），这是从什么语言继承来的
-- 4. 用 Lua 实现一个三元运算符的替代函数：function ternary(cond, a, b)，根据条件返回 a 或 b
-- 5. 计算表达式：1 + 2 * 3 ^ 2 - 10 // 3 % 4 的结果，并说明运算顺序
